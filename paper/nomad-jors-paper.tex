...
The dashboard provides additional analytics views for resource tracking and infrastructure monitoring. The resource analytics view (Figure~\ref{fig:resources}) shows CPU and GPU usage by group and user, job submission patterns as a heatmap, and active interactive computing sessions. The infrastructure monitoring view (Figure~\ref{fig:infrastructure}) extends visibility to departmental workstations and storage servers, enabling correlation of job failures with infrastructure-wide issues.

\begin{figure}[htbp]
\centering
\includegraphics[width=0.85\textwidth]{figures/resource-analytics.png}
\caption{Resource analytics views. \textbf{Top}: Resource usage summary showing CPU-hours, GPU-hours, and usage breakdown by group and user. \textbf{Middle}: Activity heatmap displaying job submissions by day and hour, revealing usage patterns (busiest: Monday 11:00). \textbf{Bottom}: Interactive computing sessions showing active RStudio and Jupyter sessions with memory usage and session age.}
\label{fig:resources}
\end{figure}
